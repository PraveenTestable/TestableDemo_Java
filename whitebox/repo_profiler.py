"""Repository profiling and Java repo data generation."""
...
def profile_language(language: str = SUPPORTED_LANGUAGE, base_dir: Path | None = None) -> dict[str, Any]:
    """Build a deterministic Java profile from sample source files."""
...
def run_initial_whitebox() -> dict[str, Any]:
    """Whitebox initial run: profile Java and write baseline + current snapshots."""
    languages = detect_languages()
    if not languages:
        raise RuntimeError(f"No Java sources found under {LANGUAGE_SAMPLE_DIR}")
...
def update_current_repo_data(language: str = SUPPORTED_LANGUAGE) -> dict[str, Any]:
    """Re-profile Java after a commit and refresh current snapshot."""
