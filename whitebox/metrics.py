...
# Minimum acceptable threshold for each metric (used for validation).
METRIC_THRESHOLDS = {
...
class ControlFlowMetrics:
    ...
    def from_dict(cls, data: dict[str, Any]) -> "ControlFlowMetrics":
        ...
        if missing:
            raise ValueError(f"Invalid metric data — missing keys: {missing}")
        try:
            return cls(**{key: float(data[key]) for key in METRIC_KEYS})
        except (TypeError, ValueError) as exc:
            raise ValueError(f"Invalid metric data — non-numeric value: {exc}") from exc
    ...
    def validate(self) -> list[str]:
        ...
        for key in METRIC_KEYS:
            ...
            if value < METRIC_THRESHOLDS[key]:
                failing.append(key)
        return failing
...
def compute_composite_score(metrics: ControlFlowMetrics) -> float:
    ...
    Raises ValueError if any individual metric is out of the [0, 100] range.
    """
    for key in METRIC_KEYS:
        value = getattr(metrics, key)
        if not (0.0 <= value <= 100.0):
            raise ValueError(
                f"Metric '{key}' value {value} is out of range [0, 100]"
            )
    total = 0.0
    for key in METRIC_KEYS:
