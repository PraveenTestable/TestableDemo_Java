# Whitebox Repo Demo — Control Flow Metrics (Java)

Sample **Java-only** repository for **Testable Whitebox initial run** and **commit-triggered score updates**.

## Scenario

1. **Initial run** — Profile the repo, compute Control Flow metrics, and persist a snapshot under `repo_data/initial_run/java.json`.
2. **Commit trigger** — On each commit, re-profile changed files, update `repo_data/current/java.json`, recompute metric deltas, and append to `repo_data/score_history.json`.
