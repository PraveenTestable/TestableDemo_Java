...
public final class CoverageVerification {
    ...
    public static void verifyAllHttpMethods() {
        PaymentRouter router = new PaymentRouter();
        assert "405".equals(router.route("DELETE", true, false));
        assert "405".equals(router.route("PATCH", true, false));
        assert "405".equals(router.route("HEAD", false, false));
        assert "405".equals(router.route("OPTIONS", true, false));
        assert "405".equals(router.route("TRACE", false, false));
    }

    public static void verifyThrottlingBehavior() {
        PaymentRouter router = new PaymentRouter();
        assert "401-unauthorized".equals(router.route("GET", true, true));
        assert "401-unauthorized".equals(router.route("POST", true, true));
        assert "401-unauthorized".equals(router.route("PUT", true, true));
        assert "401-unauthorized".equals(router.route("GET", false, true));
        assert "401-unauthorized".equals(router.route("POST", false, false));
    }
    ...
    public static void runAllVerifications() {
        ...
        verifyAllHttpMethods();
        verifyThrottlingBehavior();
    }
}
