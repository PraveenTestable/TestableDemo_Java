"""
Exhaustive tests for whitebox/repo_profiler.py — all execution paths.
"""
...
def test_detect_languages_finds_java() -> None:
    langs = detect_languages()
    assert langs == [SUPPORTED_LANGUAGE]


def test_detect_languages_missing_dir(tmp_path) -> None:
    assert detect_languages(base_dir=tmp_path / "no-such-dir") == []


def test_detect_languages_empty_dir(tmp_path) -> None:
    (tmp_path / SUPPORTED_LANGUAGE).mkdir(parents=True)
    assert detect_languages(base_dir=tmp_path) == []


def test_detect_languages_with_java_file(tmp_path) -> None:
    java_dir = tmp_path / SUPPORTED_LANGUAGE
    java_dir.mkdir(parents=True)
    (java_dir / "Foo.java").write_text("public class Foo {}")
    assert detect_languages(base_dir=tmp_path) == [SUPPORTED_LANGUAGE]


def test_count_pattern_finds_matches() -> None:
    assert _count_pattern("if (x) { if (y) {} }", r"\bif\b") == 2


def test_count_pattern_no_matches_returns_zero() -> None:
    assert _count_pattern("int x = 1;", r"\bif\b") == 0
...
def test_profile_language_unsupported_raises() -> None:
    with pytest.raises(ValueError, match="Unsupported"):
        profile_language("python")


def test_profile_language_has_positive_loc() -> None:
    assert profile_language(SUPPORTED_LANGUAGE)["lines_of_code"] > 0
...
def test_profile_language_counts_test_assertions() -> None:
    assert profile_language(SUPPORTED_LANGUAGE)["test_assertions"] > 0
...
def test_build_language_repo_data_domain() -> None:
    assert build_language_repo_data(SUPPORTED_LANGUAGE)["whitebox_domain"] == "control_flow_testing"
...
def test_write_repo_data_creates_file(tmp_path, monkeypatch) -> None:
    ...
    assert path.exists()
    assert json.loads(path.read_text())["language"] == SUPPORTED_LANGUAGE
