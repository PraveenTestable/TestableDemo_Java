"""
Exhaustive tests for whitebox/metrics.py — all branches, formulae, edge cases.
"""
...
def test_covered_paths_capped_at_execution_paths() -> None:
    profile = {**_perfect_profile(), "covered_paths": 9999}
    metrics = compute_metrics_from_profile(profile)
    assert metrics.execution_path_integrity <= 100.0


def test_high_complexity_lowers_debt_score() -> None:
    low_cc = {**_perfect_profile(), "cyclomatic_complexity": 1}
    high_cc = {**_perfect_profile(), "cyclomatic_complexity": 100}
    assert (
...
def test_all_required_fields_individually(field: str) -> None:
    profile = {**_perfect_profile(), field: 0}
    metrics = compute_metrics_from_profile(profile)
    assert 0.0 <= compute_composite_score(metrics) <= 100.0
