"""
Tests for whitebox/score_engine.py and whitebox/commit_trigger.py — all paths.
"""
...
def test_load_metrics_from_valid_data() -> None:
    ...
    assert isinstance(load_metrics_from_repo_data(data), ControlFlowMetrics)
...
def test_score_delta_regressed_flag() -> None:
    low  = {k: 0.0   for k in METRIC_KEYS}
    high = {k: 100.0 for k in METRIC_KEYS}
    result = score_delta({"control_flow_metrics": high}, {"control_flow_metrics": low})
...
def test_load_language_repo_data_corrupted_raises(tmp_path, monkeypatch) -> None:
    ...
    (current_dir / "java.json").write_text("not-json")
...
def test_on_commit_zero_delta_when_unchanged(tmp_path, monkeypatch) -> None:
    ...
    result = on_commit("sha-test-001", changed_languages=["java"])
    assert result["languages"]["java"]["delta"] == 0.0
...
def test_on_commit_result_structure(tmp_path, monkeypatch) -> None:
    ...
    assert "per_metric_delta" in result["languages"]["java"]
...
def test_on_commit_detects_code_change(tmp_path, monkeypatch) -> None:
    ...
    try:
        ...
        result = on_commit("sha-delta-001", changed_languages=["java"])
        assert result["languages"]["java"]["delta"] != 0.0
    finally:
        for path, content in originals.items():
            path.write_text(content)
