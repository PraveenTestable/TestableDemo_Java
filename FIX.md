...
```bash
python3 -m whitebox.cli initial-run
python3 -m whitebox.cli on-commit --commit-sha demo-fix-001
cat repo_data/score_history.json
```

After modifying sources under `sample_code/java/`, rerun `on-commit` and confirm
